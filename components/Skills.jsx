...
const Skills = ({ skills }) => {
  return (
    ...
    >
      <div className="container border mx-auto flex flex-col gap-10 px-5 md:px-20 py-20">
        ...
        <div className="skills my-10 flex flex-wrap gap-5 justify-center items-center">
          {skills?.map(({ _id, name, bgColor, icon }) => (
            <div className="flex flex-col gap-3 items-center">
              <div key={_id} className={`relative w-32 h-32 rounded-full bg-[${bgColor}]`} title={name}>
                <img src={urlFor(icon)} alt={name} fill className="rounded-full p-3" />
              </div>
